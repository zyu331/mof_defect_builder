...
CHARGE_APP_PATH = 'python xyz2mol-master/'
        ...
        if self.linker_type[linker_type] in charge_dict.keys():
            ...
            for molecule in self.molecules:
                ...
                if molecule.formula == self.linker_type[linker_type]:
                    ...
                    def run_with_timeout(timeout, charge_guess):
                        signal.signal(signal.SIGALRM, handler)
                        signal.alarm(timeout)
                        try:
                            re = os.popen( CHARGE_APP_PATH+'xyz2mol.py temp.xyz --ignore-chiral --charge -'+str(charge_guess)).read()
                        except Exception as e:
                            if str(e) == "Timed out":
                                print("Function timed out after", timeout, "seconds")
                                re = []
                        finally:
                            signal.alarm(0)
                        return re
                    ...
                    if len(O_count)!= 0:
                        re = run_with_timeout(3, int(len(O_count)/2))
                        if len(re) > 0:
                    ...
                    for trial_charge in range(0,6):
                        re = run_with_timeout(3, trial_charge)
                        if len(re) > 0:
                            charge_comp = -trial_charge
                            success = True
                            break
                    try:
                        test_linker_xyz = Chem.MolFromXYZFile('temp.xyz')
                        print(Chem.MolToMolBlock(test_linker_xyz),file=open('temp.mol','w+'))
                        m=Chem.MolFromMolFile('temp.mol')
                        AllChem.ComputeGasteigerCharges(m)
                        charge_comp = int(sum([atom.GetDoubleProp('_GasteigerCharge') for atom in m.GetAtoms()]))
